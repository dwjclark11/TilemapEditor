...
class MouseControl
{
  private:
	...
	// Private functions
  private:
	void MouseBox( const AssetManager_Ptr& assetManager, Renderer& renderer, SDL_Rect& mouseBox, SDL_Rect& camera,
				   bool collider = false );
  ...
  public:
	...
	*/
	void CreateTile( const AssetManager_Ptr& assetManager, Renderer& renderer, struct SDL_Rect& mouseBox,
					 struct SDL_Rect& camera, union SDL_Event& event );

	/*
	 *	Creates a Box Collideron the canvas.
	 *	@param - AssetManager_Ptr, not really needed; however, the mousebox function uses the AssetManager for creating
	 *tiles.
	 *	@param - Renderer& this is a wrapper for the SDL_Renderer, this is needed to be passed into the Mousebox
	 *function
	 *	@param - SDL_Rect& mousebox - the mouse box is the current size of the sprite you are tiling [16x16], [32x32]
	 *etc
	 *	@param - SDL_Rect& camera - The main camera to offset the position of the tile in the 2D world
	 *	@param - SDL_Event& event - This is used to know when the mouse was pressed
	 */
	void CreateCollider( const AssetManager_Ptr& assetManager, Renderer& renderer, struct SDL_Rect& mouseBox,
						 struct SDL_Rect& camera, union SDL_Event& event );
	...
	*/
	void SetSpriteProperties( const std::string& assetID, const int width, const int height, const int layer,
	...
	void SetAnimationProperties( const int numFrames, const int frameSpeedRate, bool vertical, bool looped,
								 int frameOffset );
	/*
	...
	/*
	 *  The PanCamera function takes care of the calculations for panning the camera. Around. We want to be able to move
	 * the camera around with the mouse without causing coordinate problems for new tiles created.
	 *  @params - SDL_Rect& camera - Takes in the main camera reference to change the camera position
	 *  @params - AssetManager_Ptr and Renderer - Used to create the "HAND" texture when panning using the middle mouse
	 * button
	 */
