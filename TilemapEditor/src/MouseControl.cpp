...
void MouseControl::MouseBox( const AssetManager_Ptr& assetManager, Renderer& renderer, SDL_Rect& mouseBox,
							 SDL_Rect& camera, bool collider )
{
	...
	SDL_Rect srcRect = { static_cast<int>( mSpriteComponent.mSrcRect.x ),
						 static_cast<int>( mSpriteComponent.mSrcRect.y ),
						 static_cast<int>( mMouseRect.x ),
						 static_cast<int>( mMouseRect.y ) };

	SDL_Rect dstRect = {
		static_cast<int>( mouseBox.x ),
		static_cast<int>( mouseBox.y ),
		static_cast<int>( std::round( mouseBox.w * mMouseRect.x * mTransformComponent.mScale.x * mZoom ) ),
		static_cast<int>( std::round( mouseBox.h * mMouseRect.y * mTransformComponent.mScale.y * mZoom ) ) };
...
MouseControl::MouseControl()
	...
	, mSpriteComponent()
	, mTransformComponent()
	, mRemovedTransformComponent()
	, mBoxColliderComponent()
	, mRemovedBoxComponent()
	, mAnimationComponent()
	, mRemovedAnimationComponent()
{
}

void MouseControl::CreateTile( const AssetManager_Ptr& assetManager, Renderer& renderer, SDL_Rect& mouseBox,
							   SDL_Rect& camera, SDL_Event& event )
{
	...
	glm::vec2 pos = glm::vec2( mouseBox.x + camera.x / mGridSize, mouseBox.y + camera.y / mGridSize );

	// Set the transform position to the current mousebox position
	mTransformComponent.mPosition = glm::vec2( mouseBox.x + camera.x, mouseBox.y + camera.y );
	...
	if ( ( event.type == SDL_MOUSEBUTTONDOWN || LeftButtonDown() ) && !mOverImGuiWindow )
	{
		...
		if ( ( event.button.button == SDL_BUTTON_LEFT && !mLeftPressed ) || FastTile( pos ) )
		{
			...
			else
			{
				mTransformComponent.mPosition.x =
					static_cast<int>( mMousePosScreen.x - ( mMouseRect.x * mTransformComponent.mScale.x / 2 ) );
				mTransformComponent.mPosition.y =
					static_cast<int>( mMousePosScreen.y - ( mMouseRect.y * mTransformComponent.mScale.y / 2 ) );
			}
			...
			tile.AddComponent<SpriteComponent>( mSpriteComponent.mAssetId,
			...
			if ( mIsCollider )
			{
				tile.AddComponent<BoxColliderComponent>(
					mBoxColliderComponent.mWidth, mBoxColliderComponent.mHeight, mBoxColliderComponent.mOffset );
			}

			if ( mIsAnimated )
			{
				tile.AddComponent<AnimationComponent>( mAnimationComponent.mNumFrames,
		...
		if ( event.button.button == SDL_BUTTON_RIGHT && !mOverImGuiWindow && !mRightPressed )
		{
			...
			glm::vec2 subtract = glm::vec2( ( mMouseRect.x * mTransformComponent.mScale.x ) / 2,
			...
			for ( auto& entity : entities )
			{
				...
				if ( mMousePosX >= transform.mPosition.x &&
					 mMousePosX <= transform.mPosition.x + sprite.mWidth * transform.mScale.x &&
					 mMousePosY >= transform.mPosition.y &&
					 mMousePosY <= transform.mPosition.y + sprite.mHeight * transform.mScale.y &&
...
void MouseControl::CreateCollider( const AssetManager_Ptr& assetManager, Renderer& renderer, SDL_Rect& mouseBox,
								   SDL_Rect& camera, SDL_Event& event )
{
	...
	if ( event.type == SDL_MOUSEBUTTONDOWN && !mLeftPressed )
	{
		if ( event.button.button == SDL_BUTTON_LEFT && !mOverImGuiWindow )
		{
			...
			boxCollider.AddComponent<TransformComponent>( mTransformComponent.mPosition / glm::vec2( mZoom, mZoom ),
														  mTransformComponent.mScale,
														  mTransformComponent.mRotation );

			boxCollider.AddComponent<BoxColliderComponent>(
				mBoxColliderComponent.mHeight, mBoxColliderComponent.mWidth, mBoxColliderComponent.mOffset );
			mLeftPressed = true;
		}
		...
		if ( event.button.button == SDL_BUTTON_RIGHT && !mOverImGuiWindow )
		{
			...
			for ( auto& entity : entities )
			{
				...
				if ( mMousePosX >= transform.mPosition.x &&
					 mMousePosX <= transform.mPosition.x + box_collider.mWidth * transform.mScale.x &&
					 mMousePosY >= transform.mPosition.y &&
					 mMousePosY <= transform.mPosition.y + box_collider.mHeight * transform.mScale.y )
				{
					entity.Kill();
					mRightPressed = true;
...
void MouseControl::SetSpriteProperties( const std::string& assetID, const int width, const int height, const int layer,
...
void MouseControl::SetAnimationProperties( const int numFrames, const int frameSpeedRate, bool vertical, bool looped,
										   int frameOffset )
{
...
void MouseControl::PanCamera( SDL_Rect& camera, const float& dt, const AssetManager_Ptr& assetManager,
							  Renderer& renderer )
{
	if ( MiddleButtonDown() )
	{
		...
		SDL_Rect dstRect{ static_cast<int>( mMousePosX * mZoom - camera.x ),
						  static_cast<int>( mMousePosY * mZoom - camera.y ),
						  48,
						  48 };
		// Draw the mouse hand image when using the panning function
		SDL_RenderCopyEx( renderer.get(),
						  assetManager->GetTexture( "mouse_hand" ).get(),
						  &srcRect,
						  &dstRect,
						  NULL,
						  NULL,
