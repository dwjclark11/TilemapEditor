...
void Application::Init()
{
	...
	// Create the window
	mWindow = Window( SDL_CreateWindow( "JADE Tilemap Editor",
	...
	// Create the renderer
	mRenderer =
		Renderer( SDL_CreateRenderer( mWindow.get(), -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC ) );
...
void Application::Zoom( SDL_Event& event )
{
	...
	// if (x > halfWidth)
	//{
	//	mCamera.x += ((x - halfWidth) * mZoom);
	// }
	// else if (x < halfWidth)
	//{
	//	mCamera.x -= ((halfWidth - x) * mZoom);
	// }

	// if (y > halfHeight)
	//{
	//	mCamera.y += ((y - halfHeight) * mZoom);
	// }
	// else if (y < halfHeight)
	//{
	//	mCamera.y -= ((halfHeight - y) * mZoom);
	// }

	LOG_INFO( "X: {0}, Y: {1}", x, y );
	LOG_INFO( "CAM_X: {0}, CAM_Y: {1}", mCamera.x, mCamera.y );
	LOG_INFO( "WIDTH: {0}, HEIGHT: {1}", halfWidth, halfHeight );
}
