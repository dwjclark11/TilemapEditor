...
void RenderGuiSystem::Update( const AssetManager_Ptr& assetManager, Renderer& renderer, SDL_Rect& mouseBox,
							  SDL_Rect& camera, SDL_Event& event, const float& zoom, const float& dt )
{
	ImGui::NewFrame();
			...
			if ( ImGui::InputInt( "Canvas Width", &mCanvasWidth, mTileSize, mTileSize ) )
			{
				...
				if ( mPrevCanvasWidth != mCanvasWidth )
				{
					...
					mCommandManager->ExecuteCmd(
						std::make_shared<ChangeCanvasSizeCommand>( mCanvas, mPrevCanvasWidth, mPrevCanvasHeight ) );
...
void RenderGuiSystem::RenderGrid( Renderer& renderer, SDL_Rect& camera, const float& zoom )
{
	...
	for ( int i = 0; i < yTiles; i++ )
	{
		for ( int j = 0; j < xTiles; j++ )
		{
			...
			SDL_Rect newRect{ static_cast<int>( ( std::floor( j * mTileSize * zoom ) ) - camera.x ),
							  static_cast<int>( ( std::floor( i * mTileSize * zoom ) ) - camera.y ),
							  static_cast<int>( std::ceil( mTileSize * zoom ) ),
							  static_cast<int>( std::ceil( mTileSize * zoom ) ) };
...
void RenderGuiSystem::ShowMouseLocationText( SDL_Rect& mouseBox, SDL_Rect& camera )
{
	...
	if ( !mMouseControl->MouseOutOfBounds() && ( mCreateTiles || mCreateColliders ) )
	{
		...
		if ( mGridSnap )
			ImGui::TextColored(
				ImVec4( 0, 255, 0, 1 ), "Mouse [X: %d, Y: %d]", mGridX * mTileSize, mGridY * mTileSize );
		else // Print the location of the mouse relative to the canvas
			ImGui::TextColored( ImVec4( 0, 255, 0, 1 ),
								"Mouse [X: %d, Y: %d]",
