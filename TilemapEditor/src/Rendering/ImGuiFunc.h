...
class ImGuiFuncs
{
  private:
	...
	{
		int scaleX, scaleY, boxWidth, boxHeight, boxOffsetX, boxOffsetY, layer, srcRectX, srcRectY, mouseRectX,
			mouseRectY;
		int numFrames, frameSpeed, frameOffset;
		bool vertical, looped;

		TileProperties( int scale_x = 1, int scale_y = 1, int box_width = 16, int box_height = 16, int box_offset_x = 0,
						int box_offset_y = 0, int layer = 0, int src_rect_x = 0, int src_rect_y = 0,
						int mouse_rect_x = 16, int mouse_rect_y = 16, int num_frames = 1, int frame_speed = 10,
						int frame_offset = 0, bool vertical = false, bool looped = true )
		{
			this->scaleX = scale_x;
			this->scaleY = scale_y;
  ...
  public:
	 ...
	 */
	void ShowFileMenu( sol::state& lua, const AssetManager_Ptr& assetManager, Renderer& renderer,
	 ...
	 */
	void ShowTileProperties( std::shared_ptr<class MouseControl>& mouseControl, const AssetManager_Ptr& assetManager,
							 bool collider );
	 ...
	 */
	void UpdateShortCuts( sol::state& lua, const AssetManager_Ptr& assetManager, Renderer& renderer,
						  std::shared_ptr<Canvas>& canvas, int& tileSize,
	 ...
	 */
	void OpenProject( sol::state& lua, const AssetManager_Ptr& assetManager, Renderer& renderer,
					  std::shared_ptr<Canvas>& canvas, int& tileSize );
	 ...
	 */
	void Save( const AssetManager_Ptr& assetManager, Renderer& renderer, const int& canvasWidth,
