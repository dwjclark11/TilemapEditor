...
void FileLoader::LoadProject( sol::state& lua, const std::string& filename, const AssetManager_Ptr& assetManager,
							  Renderer& renderer, std::vector<std::string>& assetIds,
							  std::vector<std::string>& assetFilepaths, std::shared_ptr<Canvas>& canvas, int& tileSize )
{
	/*
...
void FileLoader::LoadMap( const AssetManager_Ptr& assetManager, const std::string& filename )
{
	...
	while ( true )
	{
		...
		mapFile >> group >> assetID >> tileWidth >> tileHeight >> srcRectX >> srcRectY >> layer >> transform.x >>
			transform.y >> scale.x >> scale.y >> collider;
...
void FileLoader::SaveMap( std::filesystem::path filename )
{
	...
	if ( !mapFile.is_open() )
	{
		LOG_ERROR( "FILELOADER__LINE__97: Unable to open[{0}] for saving", filename.string() );
		return;
	}
	...
	for ( const auto& tile : tiles )
	{
		...
		mapFile << group << " " << sprite.mAssetId << " " << sprite.mWidth << " " << sprite.mHeight << " "
				<< sprite.mSrcRect.x << " " << sprite.mSrcRect.y << " " << sprite.mLayer << " " << transform.mPosition.x
				<< " " << transform.mPosition.y << " " << transform.mScale.x << " " << transform.mScale.y << " ";
		...
		if ( collider )
		{
			...
			mapFile << collider << " " << boxCollider.mWidth << " " << boxCollider.mHeight << " "
					<< boxCollider.mOffset.x << " " << boxCollider.mOffset.y << " "; // << std::endl;
		}
		else
		{
			collider = false;
		...
		if ( animated )
		{
			...
			mapFile << animated << " " << animation.mNumFrames << " " << animation.mFrameSpeedRate << " "
					<< animation.mVertical << " " << animation.mIsLooped << " " << animation.mFrameOffset << " "
					<< std::endl;
		}
		else
		{
...
void FileLoader::SaveColliders( std::filesystem::path filename )
{
	...
	if ( !mapFile.is_open() )
	{
		LOG_ERROR( "FILELOADER__LINE__230: Unable to open[{0}] for saving", filename.string() );
		return;
	}
	...
	for ( const auto& collider : colliders )
	{
		...
		// Save to the map file
		mapFile << group << " " << transform.mPosition.x << " " << transform.mPosition.y << " " << transform.mScale.x
				<< " " << transform.mScale.y << " " << boxCollider.mWidth << " " << boxCollider.mHeight << " "
				<< boxCollider.mOffset.x << " " << boxCollider.mOffset.y << " " << std::endl;
	}
	// Close the file
	mapFile.close();
...
void FileLoader::SaveProject( const std::string& filename, std::vector<std::string>& assetIds,
							  ...
							  const int& tileSize )
{
	...
	if ( !mapFile.is_open() )
	{
		LOG_ERROR( "FILELOADER__LINE__313: Unable to open[{0}] for saving", filepath.string() );
		return;
	}
	...
	{
		LOG_ERROR( "FILELOADER__LINE__327: Unable to open[{0}] for saving", filepath.string() );
		return;
	}
...
void FileLoader::SaveToLuaTable( const std::string& filename, std::vector<std::string>& assetIds,
